...
//  顺序二叉树的函数成员:
	...
	SqBinaryTree<ElemType> &operator=(const SqBinaryTree<ElemType>& t);		
		// 赋值运算符重载
	//作业:寻找最近共同祖先
	bool FindCommonAncestor(const ElemType &a,const ElemType &b,ElemType &common_ancestor)
	{
		int ai,bi,i;
		for(i=0;i<maxSize;i++) if(elems[i]==a && tag[i])
		{
			ai=i;
			break;
		}
		...
		for(i=0;i<maxSize;i++) if(elems[i]==b && tag[i])
		{
			bi=i;
			break;
		}
