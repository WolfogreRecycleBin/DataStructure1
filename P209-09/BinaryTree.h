...
// 求以r为根的二叉树的结点个数
	...
	//作业辅助:求最大宽度计数
	void WidthCount(BinTreeNode<ElemType>*r,int generation,int width[]) const
	{
		...
		if (r != NULL)
		{
			width[generation]++;
			generation++;
			WidthCount(r->leftChild, generation,width);
			WidthCount(r->rightChild, generation,width);
		}
	}
public:
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	int MaxWidth()
	{
		...
		int *width = new int[node_count];
		for(int i=0;i<node_count;i++) width[i]=0;
		WidthCount(root,0,width);
		int max_width=0;
		for(int i=0;width[i]!=0 && i<node_count;i++) if(max_width<width[i]) max_width=width[i];
		delete [] width;
		return max_width;
	}
